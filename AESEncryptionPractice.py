'''
Purely for learning purposes only, actual encryption not objective or realistic

Created by Hugh Ferguson on 7/13/2017
'''
import random

'''
KEY EXPANSION
Effectively encrypting the key itself

Each round uses modified version of the key
'''

'''
INITIAL ROUND
Inputs - Message and expanded key (quantity: 10)
AddRoundKey(whitening)(XOR)
'''

'''
ROUNDS
128 bits = 10 rounds (smaller key size for testing purposes)

SubBytes
ShiftRows
MixColumns
AddRoundKey

'''


def GetMessageAndKey():

    message = raw_input("What is your message? ")
    key = random.getrandbits(16)

    state = message.split()

    return state, key


def SubBytes(state):

    SBox = (
        
            )

    for x in range(0, len(state)):
        state[x] = SBox[x]


def rounds(count, state, key):

    SubBytes(state)      # Main rounds (repeated x times)
    ShiftRows(state)
    MixColumns()
    AddRoundKey(state, key)


def AES():

    state, key = GetMessageAndKey()

    KeyExpansion()
    AddRoundKey()   # Same as initial round

    Rounds(1)       # Encryption rounds repeated x times
                    # (1 for learning purposes)

    SubBytes(state)      # Final round
    ShiftRows(state)
    AddRoundKey(state, key)


def main():
    AES()


if __name__ == "__main__":
    main()
